...
@interface SKFDownLoader ()<NSURLSessionDataDelegate>
{
    // 临时文件的大小
...
+ (NSString *)downLoadedFileWithURL: (NSURL *)url {
    ...
    if([SKFDownLoaderFileTool isFileExists:cacheFilePath]) {
...
- (void)downLoadWithURL: (NSURL *)url {
    ...
    // 文件的位置, 文件的大小
    if ([SKFDownLoaderFileTool isFileExists:self.cacheFilePath]) {
        NSLog(@"文件已经下载完毕, 直接返回相应的数据--文件的具体路径, 文件的大小");
        
        if (self.downLoadInfo) {
            self.downLoadInfo([SKFDownLoaderFileTool fileSizeWithPath:self.cacheFilePath]);
        }
        
        self.state = SKFDownLoaderStateSuccess;
    ...
    if ([url isEqual:self.task.originalRequest.URL]) {
        // 任务存在 -> 状态
        // 状态 -> 正在下载 返回
        if (self.state == SKFDownLoaderStateDowning)
        {
            return;
        }
        // 状态 -> 暂停 = 恢复
        if (self.state == SKFDownLoaderStatePause)
        {
            [self resume];
            return;
        }
    ...
    [self cancel];
    // 2. 读取本地的缓存大小
    _tmpFileSize = [SKFDownLoaderFileTool fileSizeWithPath:self.tmpFilePath];
...
- (void)resume {
    if (self.state == SKFDownLoaderStatePause) {
       [self.task resume];
        self.state = SKFDownLoaderStateDowning;
    }
}
...
- (void)pause {
     ...
     {
        [self.task suspend];
         self.state = SKFDownLoaderStatePause;
     }
...
//    self.state = SKFDownLoaderStateFailed;
}
...
- (void)cancelAndClearCache {
    ...
    // 删除缓存
    [SKFDownLoaderFileTool removeFileAtPath:self.tmpFilePath];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    if (_tmpFileSize == _totalFileSize) {
        NSLog(@"文件已经下载完成, 移动数据");
        // 移动临时缓存的文件 -> 下载完成的路径
        [SKFDownLoaderFileTool moveFile:self.tmpFilePath toPath:self.cacheFilePath];
        self.state = SKFDownLoaderStateSuccess;
        // 取消请求
...
- (void)setState:(SKFDownLoaderState)state {
    if (_state == state) {
        return;
    }
    _state = state;
